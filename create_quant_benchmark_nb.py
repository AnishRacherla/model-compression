...
# ── Cell 5: Baseline int4 ─────────────────────────────────────────────────────
cells.append({
 "cell_type": "markdown",
 "metadata": {},
 "source": ["## 🔵 Step 1: Baseline — Original Aya Expanse 8B in int4 (no pruning, no finetuning)"]
})
cells.append({
 "cell_type": "code",
 "execution_count": None,
...
# ── Cell 7: Merge LoRA and push merged fp16 to Hub (NO local disk save) ───────
cells.append({
 "cell_type": "markdown",
 "metadata": {},
 "source": ["## 🟡 Step 2: Merge pruned fp16 + LoRA → push to Hub (no disk write)"]
})
cells.append({
 "cell_type": "code",
 "execution_count": None,
...
# ── Cell 9: Compressed int4 from Hub ─────────────────────────────────────────
cells.append({
 "cell_type": "markdown",
 "metadata": {},
 "source": ["## 🟢 Step 3: Load merged from Hub → int4 → benchmark → push to int4 repo"]
})
cells.append({
 "cell_type": "code",
 "execution_count": None,
...
# ── Cell 11: Compressed int8 from Hub ────────────────────────────────────────
cells.append({
 "cell_type": "markdown",
 "metadata": {},
 "source": ["## 🟠 Step 4: Load merged from Hub → int8 → benchmark → push to int8 repo"]
})
cells.append({
 "cell_type": "code",
 "execution_count": None,
...
# ── Cell 13: COMET scoring ────────────────────────────────────────────────────
cells.append({
 "cell_type": "markdown",
 "metadata": {},
 "source": ["## 📊 Step 5: COMET Scoring (all 3 models in isolated subprocess)"]
})
cells.append({
 "cell_type": "code",
 "execution_count": None,
...
# ── Cell 15: Final table + update Hub READMEs ─────────────────────────────────
cells.append({
 "cell_type": "code",
 "execution_count": None,
 "metadata": {},
...
print("✅ quantization_benchmark.ipynb created (zero disk usage design)!")
